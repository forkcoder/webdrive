...
$session = new DBProxy();
if ($_GET['auth_ph']!=$_SESSION['login_key'] ||  isset($_SESSION['fcoder_remember_token'])== false) {
	...
	if ($faultyData == false) {
		...
		if (mysqli_num_rows($result) == 1) {
			...
			$sql = "UPDATE fcoder_users  set gspace= '$gspace', lspace='$lspace', role='$role', name='$name', userid='$userid', contact_no='$contactno', updated_at = '$today' , userid='$userid', actype='$actype', password_hash='$password' , genid ='$genid' where  id=$id and email_id='$email_id'";
