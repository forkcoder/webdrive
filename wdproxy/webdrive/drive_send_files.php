...
if ($session->remote_validate($_POST['access_key'])){
  ...
  if ($_SESSION['vlan_features']['file_transfer'] == 1 && $_SESSION['fcoder_ftransfer_access'] == 1) {
                ...
                foreach ($selectedUsers as $sendto) {
                  ...
                  if (mysqli_num_rows($result) == 1) {
                    ...
                    $sqls = "INSERT INTO fcoder_file_transfer (f_sender_genid, f_sender_name,f_receiver_genid, f_random_sid, f_send_date_time, f_status, f_send_comments, f_send_hostname, f_send_ipaddress,f_file_name,f_file_size)
                       VALUES('$u_genid','$u_name','$sendto','$random_sid','$time','Pending','File from Web Drive','$hostname','$ipaddress','$fname',$file_size);";
                    $result = mysqli_query($con, $sqls) or die("File Transfer info could not be done");
                    $frealpath = str_replace ("'","''",$realpath);
                    $sql = "INSERT into fcoder_webdrive_log (wdl_action, wdl_iuser_id, wdl_src, wdl_dest, wdl_datetime, wdl_status,wdl_msg)
                    values('send', '$u_genid', '$frealpath', '$location', '$time',1,200)";
