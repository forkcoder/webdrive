...
{
	targetEl = false;
	//document.write(document.getElementById(tgtEl));
	...
	calTable.style.top =  (positions[1]+20)+'px';
...
function closeCalender(validDate)
{
	...
	if(validDate==false){
		if(targetEl.getAttribute('id')=="i_startdate"){
			targetEl.value = 'Start Date';
			reportModule.setStartdate('');
			document.getElementById('i_enddate').value= 'End Date';
			reportModule.setEnddate('');
		}
		else if(targetEl.getAttribute('id')=="i_enddate"){
			targetEl.value = 'End Date';
			reportModule.setEnddate('');
		}
	}
}
...
// function getDayCount(a){
// 	var oneDay = 24*60*60*1000;
// 	return Math.round((a.getTime())/(oneDay));
// }
...
function pickDate(year, month, day)
{
	...
	if (!targetEl)
	{
		showNotificationMsg('alert', 'target for date is not set yet');
	}
	else
	{
		var curDate = new Date();
		...
		if(targetEl.getAttribute('id')=="i_startdate"){
			var endDate = reportModule.getEnddate();
			if(getDateDiff(currentDate, selectedDate)<0)
    			showNotificationMsg('alert', 'Start Date must be less than or equal to Today.');
			else if(endDate!='' && getDateDiff(new Date(endDate), selectedDate)<0)
				showNotificationMsg('alert', 'Start Date must be less than or equal to End Date.');
			else{
				targetEl.value= selectDay;
				reportModule.setStartdate(selectDay);
			}
		}
		else if(targetEl.getAttribute('id')=="i_enddate"){
			var startDate=reportModule.getStartdate();
			if (startDate!=''){
				if(getDateDiff(new Date(startDate), selectedDate)>0)
					showNotificationMsg('alert', 'End Date must be greater than or equal to Start Date.');
				else{
					if(getDateDiff(currentDate, selectedDate)<0)
		    			showNotificationMsg('alert', 'End Date must be less than or equal to Today.');
					else{
						targetEl.value= selectDay;
						reportModule.setEnddate(selectDay);
					}
				}
			}
			else
				showNotificationMsg('alert', 'Start Date must be selected at first.');
		}
		closeCalender(true);
	}
	reportModule.getIncidents();
}
