...
class LDAP
{
    private $server = "ldap://127.0.0.1";
    private $port = "389";
    private $domain = "localhost";
    private $admin = "admin";
    private $password = "";

    public function __construct($server, $port, $domain, $admin = "", $password = "")
    {
        $this->server = $server;
        $this->port = $port;
        $this->domain = $domain;
    ...
    public function authenticate($user)
    {
        ...
        $ldap = ldap_connect($this->server,$this->port) or $user->auth_status = AuthStatus::SERVER_FAIL;
    ...
    public function get_users()
    {
        ...
        $base_dn= "OU=ITMaintenance".",".$base_dn;

        $sr=ldap_search($ldap, $base_dn, "(&(objectClass=user)(objectCategory=person)(|(mail=*)(telephonenumber=*))(!(userAccountControl:1.2.840.113556.1.4.803:=2)))", array("cn", "dn", "memberof", "mail", "telephonenumber", "othertelephone", "mobile", "accountno", "lspace", "title","pager","description"));
        ...
        for($i = 0; $i < $info["count"]; $i++)
        {
            ...
            $users[$i]["skype"] = $info[$i]["accountno"][0];
            $users[$i]["telephone"] = $info[$i]["telephonenumber"][0];
            $users[$i]["lspace"] = $info[$i]["lspace"][0];
            $users[$i]["title"] = $info[$i]["title"][0];
            $user->pager = $info[0]["pager"][0];
    ...
    private function _get_user_info($ldap, $user)
    {
        ...
        $sr=ldap_search($ldap, $base_dn, "(&(objectClass=user)(objectCategory=person)(samaccountname=".$user->username."))", array("cn", "dn", "memberof", "mail", "telephonenumber", "othertelephone", "mobile", "accountno", "lspace", "title","pager","description"));
        $info = ldap_get_entries($ldap, $sr);
       ...
       // $user->skype = $info[0]["accountno"][0];
        $user->lspace = $info[0]["lspace"][0];
